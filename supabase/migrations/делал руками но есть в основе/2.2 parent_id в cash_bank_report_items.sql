-- Шаг 1: Добавление поля parent_id для поддержки иерархии в отчете по ДС.
-- Команда выполнится только если колонка еще не существует, что делает скрипт безопасным для повторного запуска.
...
-- Шаг 2: Обновление функции для загрузки отчетов, чтобы она создавала связи "родитель-потомок".
-- Функция теперь работает в два прохода: сначала вставляет все строки, а затем обновляет parent_id.
