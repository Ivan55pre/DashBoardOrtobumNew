...
CREATE TABLE organization_members (
    ...
    role TEXT NOT NULL CHECK (role IN ('admin', 'member')), -- Пример ролей
...
-- 21.07.25 если есть
-- DROP POLICY "Admins can update member roles in their organization." ON public.organization_members;

CREATE POLICY "Admins can update member roles in their organization."
ON public.organization_members
FOR UPDATE
USING (
  -- Чтобы инициировать обновление, текущий пользователь должен быть администратором
  -- в организации, к которой принадлежит участник.
  public.is_admin_of(organization_id, auth.uid())
)
WITH CHECK (
  -- При обновлении должны выполняться следующие условия:
  -- 1. Пользователь, выполняющий обновление, должен быть администратором целевой организации (на случай перемещения).
  public.is_admin_of(organization_id, auth.uid()) AND
  (
    -- 2. ЛИБО обновляемый участник НЕ является последним администратором в своей ИСХОДНОЙ организации.
    --    Здесь используется OLD, чтобы сослаться на значения до обновления.
    NOT public.is_last_admin_in_organization(OLD.organization_id, OLD.user_id)
  )
  OR
  (
    -- 3. ЛИБО он ЯВЛЯЕТСЯ последним администратором, и в этом случае
    --    его роль должна остаться 'admin', и он должен остаться в той же организации.
    --    Это предотвращает как понижение в должности, так и "перемещение" из организации.
